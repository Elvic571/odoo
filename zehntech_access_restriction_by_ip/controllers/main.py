...
class HomeCustom(Home):
    ...
    def web_login(self, redirect=None, **kw):
        ...
        if request.httprequest.method == 'POST':
            ...
            if request.params['login']:
                ...
                if user_rec.allowed_ips:
                            ...
                            if rec.ip_range_start and rec.ip_range_end:
                                try:
                                    if ':' in rec.ip_range_start:  
                                        start_ip = ipaddress.IPv6Address(rec.ip_range_start)
                                    ...
                                    if isinstance(ip, ipaddress.IPv6Address) and isinstance(start_ip, ipaddress.IPv6Address):
                                        if start_ip <= ip <= end_ip:
                                            ip_allowed = True
                                            break
                                    elif isinstance(ip, ipaddress.IPv4Address) and isinstance(start_ip, ipaddress.IPv4Address):
                                        if start_ip <= ip <= end_ip:
                                            ip_allowed = True
                                            break
                                    else:
                                        _logger.warning("IP type mismatch: IP is %s, Range is %s", type(ip), type(start_ip))
                    ...
                    if not ip_allowed:
                        notification_enabled = request.env['ir.config_parameter'].sudo().get_param('unauthorized_notification_enabled', default=False)
                        if notification_enabled:
                            self._send_blocked_login_email(user_rec, ip_address)
                        _logger.warning("Blocked login attempt for user %s from IP %s", user_rec.login, ip_address)
                        request.update_env = old_uid
                        values['error'] = _("Not allowed to login from this IP. Please contact your administrator.")
                        self._log_login_attempt(user_rec, ip_address, request.params['login'], False)
        ...
        if not request.env['ir.config_parameter'].sudo().get_param('list_db', default=False):
    ...
    def _send_blocked_login_email(self, user_rec, ip_address):
        admin_emails = request.env['res.users'].sudo().search([('groups_id', 'in', [request.env.ref('base.group_system').id]), ('email', '!=', False)]).mapped('email')
        if not admin_emails:
        ...
        mail_template = request.env.ref('zehntech_access_restriction_by_ip.email_template_blocked_login', raise_if_not_found=False)
